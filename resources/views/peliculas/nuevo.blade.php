...
@section('contenido')
    <div class="p-8 rounded-xl shadow-xl w-1/2 mx-auto bg-gray-800">
        <form name="as" method="POST" action="{{ route('peliculas.store') }}">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="sinopsis" type="text" placeholder="Sinopsis de la pelicula" name="sinopsis">{{ @old('sinopsis') }}</textarea>
                @error('sinopsis')
                    <p class="text-red-700 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <select
                    ...
                    @foreach ($categorias as $categoria)
                        <option value="{{ $categoria->id }}" @selected($categoria->id == @old('category_id'))>{{ $categoria->nombre }}</option>
                    @endforeach
                </select>
            </div>
            @error('category_id')
            ...
            <div class="flex mb-4">
                <div class="flex items-center mr-4">
                    <input id="si" type="radio" value="SI" name="doblada" @checked(@old('doblada') == 'SI')
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="si" class="ml-2 text-sm font-bold text-green-800 dark:text-black-300">SI</label>
                </div>
                <div class="flex items-center mr-4">
                    <input id="no" type="radio" value="NO" name="doblada" @checked(@old('doblada') == 'NO')
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
